...
파일을 연속 처리해야 웜 DFA 이득을 봅니다. Linux 에서는 부모가 실제 입력의
앞쪽 파일을 파싱해 DFA 를 채운 뒤 fork 합니다. 짧은 합성 유닛만으로는
부족합니다.

새 구문이 들어올 때마다 DFA state 가 쌓이므로, 기본으로 파서 DFA 기록을
**4,096 state** 에서 멈춥니다. 이미 캐시된 결정은 그대로 쓰고, 한도 밖의
희귀 구문만 ATN 으로 풀고 표에는 남기지 않습니다. `--dfa-max-states 0` 이면
무제한입니다.
