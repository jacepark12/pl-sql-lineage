...
- **Tier 2·3 샘플** — `MERGE`, CTE, 집계/분석함수, `SELECT *`, `(+)` 조인, 변수 경유는
  합성 코퍼스 16 패키지 샘플에서 각각 100% / F1 99.3% 입니다. 21 패키지 재현은
  [scoring-runs.md](scoring-runs.md). DB 링크는 `schema.table@LINK` 로 로컬 테이블과
  구분되며 엣지 `table` / `dblink` 에 보존됩니다.
- **전체 코퍼스 30만 라인** — 실측 있음. 기본 코퍼스(201 패키지 / 300,612 라인)를
  한 프로세스에서 돌리면 파싱 201/201, 벽시계 183.5s, **1,638 라인/s**
  (웜 1,737 라인/s), 고유쌍 F1 **96.7%**. 숫자는 [scoring-runs.md](scoring-runs.md)
  JAC-160. README 의 파서 전용 웜 추정치 957 라인/s 보다 이 환경에서는 빠르고,
  실무 표기 13 라인/s 와는 자릿수가 다릅니다.
...
- ~~**`%ROWTYPE` 은 앵커를 만들지 않습니다.**~~ — `r T%ROWTYPE` 의 `r.COL` 은 선언
  앵커(또는 카탈로그에 있는 T)로 `T.COL` 에 연결합니다. `FOR rec IN c LOOP` 투영
  바인딩이 있으면 그쪽이 이깁니다. 카탈로그가 T 를 알면 없는 컬럼은 지어내지 않습니다.
- **`%TYPE` 앵커는 값 소스로 쓰지 않습니다.** `structure` 가 `TABLE.COLUMN%TYPE` 을
  수집하지만, 선언 타입을 "실제로 흐른 값" 으로 쓰면 매개변수·리터럴·다른 컬럼에서
  채운 변수에 거짓 엣지가 생깁니다. `SELECT INTO` 로 채워진 값만 사용합니다.
- ~~**`CONNECT BY` 가 조용히 탈락합니다.**~~ — sqlglot Oracle 이 파싱하면
  `START WITH` / `CONNECT BY` / `PRIOR` 컬럼을 `INDIRECT_FILTER` 로 남깁니다.
- ~~**`PIVOT` / `UNPIVOT` 이 조용히 탈락합니다.**~~ — 파싱되면 값 컬럼은 집계·언피벗
  원천으로, `FOR` 키는 `INDIRECT_FILTER` 로 매핑합니다. 매핑 실패 시
  `UNSUPPORTED_PIVOT` 진단입니다.
...
[완료] %ROWTYPE 필드 접근             r T%ROWTYPE 의 r.COL → T.COL. 커서 루프는 유지
[완료] CONNECT BY                     PRIOR/계층 컬럼 → INDIRECT_FILTER
[완료] PIVOT/UNPIVOT                  값·키 컬럼 매핑. 실패 시 UNSUPPORTED_PIVOT
...
`%TYPE` 은 의도적으로 값 소스가 아닙니다 — 선언 타입과 실제 데이터플로를 섞지 않습니다.
