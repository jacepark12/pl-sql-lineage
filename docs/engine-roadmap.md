...
| B — 문장 내부 (sqlglot) | Tier 0~3 합성 코퍼스 F1 99% 이상. DB link 미지원 | `sqlmap.py` |
...
- **Tier 2·3** — `MERGE`, CTE, 집계/분석함수, `SELECT *`, `(+)` 조인, 변수 경유는 합성
  ...
  규모를 늘린 재현은 Tier 0~1 에서만 했습니다. DB 링크는 아직 미지원입니다.
...
1. 조용한 탈락 커버리지 리포트        남은 침묵은 파라미터·파일 경계
2. 동적 SQL 진단                     지어내지 않고 UNRESOLVED 로 남기기
3. 인코딩 / CREATE OR REPLACE 없는 소스   실무 진입 지점. 12 파일 묶음 재측정과 한 묶음
4. 처리량                            실무 표기에서 13 라인/s. 30만 라인 추정치가 안 맞음
5. 규모 재현 / 전체 코퍼스 실행 / 뷰어 출력 형식 / 저장 계층
```
