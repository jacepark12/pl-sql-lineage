...
속도: ANTLR `sql_script()` 가 72% 이므로 **파일마다** 프로세스를 나누는 것은
금지에 가깝습니다. 문법 DFA 를 워커 프로세스에 유지하는 것이 1,748 라인/s 의
전제입니다. 파일이 수백 개면 `--jobs N` 워커 풀이 벽시계를 줄입니다 — 워커는
여러 파일을 연속 처리하고, Linux 에서는 부모 워밍업 후 fork 합니다. 그다음이
`extract` 12% 입니다. sqlglot 최적화는 이 코퍼스에서 이득이 거의 없습니다.
